...
class chord_type_switch(object):
    ...
    def generate_chord_type_3(self, key, scale, time, chord_duration):
        ...
        self.layer1[time][0]=int(c4_id-scales[scale][base_note]*8-key*8+self.switch_duration_id_shift(chord_duration))
        self.layer1[time][1]=int(chord_duration)
        self.layer2[time][0]=int(c4_id-scales[scale].note[base_note+2]*8-key*8+self.switch_duration_id_shift(chord_duration))
        self.layer2[time][1]=int(chord_duration)
        self.layer3[time][0]=int(c4_id-scales[scale][base_note+4]*8-key*8+self.switch_duration_id_shift(chord_duration))
        self.layer3[time][1]=int(chord_duration)
            ...
            if self.layer1[pos][0]>0:
                if self.layer4[pos][0]>0:
                    if self.layer5[pos][0]>0:
                        ...
                        temp_wav=np.fromstring(temp, dtype=np.int32)
                        temp2=wav2.readframes(wav2.getnframes())
                        temp_wav2=np.fromstring(temp2, dtype=np.int32)
                        temp3=wav3.readframes(wav3.getnframes())
                        temp_wav3=np.fromstring(temp3, dtype=np.int32)
                        temp4=wav4.readframes(wav4.getnframes())
                        temp_wav4=np.fromstring(temp4, dtype=np.int32)
                        temp5=wav5.readframes(wav5.getnframes())
                        temp_wav5=np.fromstring(temp5, dtype=np.int32)
                    ...
                    print(t2)
                    while t<t2:
                        audio[t][0]+=temp_wav[t3-1]+temp_wav2[t3-1]+temp_wav3[t3-1]
                        audio[t][1]+=temp_wav[t3]+temp_wav2[t3]+temp_wav3[t3]
                        t+=1
                        t3+=2
              ...
              #      audio[t:t2][1]+=temp_wav[1::2]+temp_wav2[1::2]+temp_wav3[1::2]
                ...
                pos+=1
        audio*=int(32767/np.max(np.abs(audio)))
        playback=sa.play_buffer(audio, 2, 2, 44100)
...
def generate_progression():
    ...
    timeline=0
    if complexity!=0:
        chord_type = random.randrange(0,complexity,1)
    else:
        chord_type=0
    chord_duration = 16-density
    while timeline<64:
...
play_button = Button(main_window, text="Play", width=10, command=chord_type_switcher.wav_to_table)
play_button.pack()
